...
# courivine branch deployment (only runs when a version git tag exists - syntax: "v1.2.3")
if [[ "$CIRCLE_BRANCH" == "courivine" ]]; then
  VERSION=$(git describe --tags | grep "^v[0-9]\+\.[0-9]\+\.[0-9]\+\.[0-9]\+-courivine$")

  if [[ "$VERSION" ]]; then
    set -e

    DOCKER_NAMESPACE=${DOCKER_NAMESPACE:-"courivine/custom"}
    ...
    echo "On the courivine branch, but no version tag was found. Skipping image deployment."
  fi
fi
