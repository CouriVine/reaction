...
if [[ "$CIRCLE_BRANCH" != "leahlovise" ]]; then
  echo "Not running a deployment branch."
  exit 0
fi

# leahlovise branch deployment (only runs when a version git tag exists - syntax: "v1.2.3")
if [[ "$CIRCLE_BRANCH" == "leahlovise" ]]; then
  VERSION=$(git describe --tags | grep "^v[0-9]\+\.[0-9]\+\.[0-9]\+\.[0-9]\+-leahlovise$")

  if [[ "$VERSION" ]]; then
    set -e

    DOCKER_NAMESPACE=${DOCKER_NAMESPACE:-"leahlovise/custom"}
    ...
    echo "On the leahlovise branch, but no version tag was found. Skipping image deployment."
  fi
fi
